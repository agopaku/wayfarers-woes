...
def verify_iphone():
    ...
    print(f"Verifying {len(files)} iPhone images...")
    ...
    for filename in files:
        ...
        try:
            with Image.open(src_path) as img:
                ...
                if not has_exif and filename != "Shweta_Orchid_Lei.jpg":
    ...
    if len(errors) == 0:
        print("\nALL IPHONE OPTIMIZATION VERIFICATION CHECKS PASSED SUCCESSFULLY! ✓✓✓")
        print("All iPhone images are <= 2048px, under 1.5MB, and EXIF metadata is preserved!")
    else:
        print("\nFOUND ERRORS DURING IPHONE VERIFICATION:")
        for err in errors:
            print(f"✗ {err}")
        exit(1)
